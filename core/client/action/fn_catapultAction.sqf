...
_asset addEventHandler ["LandedTouchDown", {
    ...
    [_arrestCables, _plane] spawn {
        params ["_arrestCables", "_plane"];
        private _startTime = serverTime;
        while {_startTime + 5 > serverTime} do {
            _plane setVelocityModelSpace [0, 0, 0];
            uiSleep 0.1;
        };

        _plane setAirplaneThrottle 0;
